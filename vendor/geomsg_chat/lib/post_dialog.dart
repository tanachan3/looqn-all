...
Future<void> showPostDialog(
  ...
  final comments = await fetchComments(context, post.id);
                              ...
                              onTap: isPosting
                                  ...
                                  : () async {
                                      ...
                                      if (text.isNotEmpty &&
                                        ...
                                        final canReply =
                                            await UserStatusGuard
                                                .ensureActiveOrRedirect(
                                                    context);
                                        if (!canReply) return;
                                        final isPenalty =
                                            await PenaltyService.isPenaltyUser(
                                                userId);
                                        if (isPenalty) {
                                          await UserStatusGuard.handlePenalty(
                                              context);
                                          return;
                                        }
...
Future<List<Post>> fetchComments(
    BuildContext context, String parentPostId) async {
  final isActive = await UserStatusGuard.ensureActiveOrRedirect(context);
  if (!isActive) {
    return [];
  }
