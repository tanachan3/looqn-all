...
import 'package:geomsg_chat/user_status_guard.dart';
import 'package:geomsg_chat/penalty_service.dart';
...
class _MapPageState extends State<MapPage> with WidgetsBindingObserver {
  ...
  Timer? _reloadTimer;
  StreamSubscription<bool>? _penaltySubscription;
  bool _penaltyHandled = false;
  ...
  Future<void> _listenPenaltyStatus() async {
    final userId = await getCurrentUserDocId();
    if (userId == null) {
      return;
    }
    await _penaltySubscription?.cancel();
    _penaltySubscription =
        PenaltyService.penaltyStream(userId).listen((isPenalty) async {
      if (!isPenalty || _penaltyHandled) {
        return;
      }
      _penaltyHandled = true;
      if (!mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(loc.penaltyLogoutMessage),
        ),
      );
      await UserStatusGuard.handlePenalty(context, showMessage: false);
    }, onError: (_) {});
  }
  ...
  Future<void> _postInitialMessagesIfNeeded() async {
    ...
    final isActive = await UserStatusGuard.ensureActiveOrRedirect(context);
    if (!isActive) return;
  ...
  Future<void> _displayPosts(List<Post> allPosts) async {
    ...
    await for (final marker in markerService.streamMarkersFromPosts(
      ...
      onMarkerTap: (post) async {
        ...
        final userId = await getCurrentUserDocId();
        if (userId != null) {
          final isPenalty = await PenaltyService.isPenaltyUser(userId);
          if (isPenalty) {
            await UserStatusGuard.handlePenalty(context);
            return;
          }
        }
                    ...
                    showModalBottomSheet(
                      ...
                      builder: (context) => FractionallySizedBox(
                        ...
                        child: ClipRRect(
                          borderRadius: const BorderRadius.vertical(
                              top: Radius.circular(24)),
                          child: UserPostsPage(userId: userId),
                        ),
                      ),
                    );
                  },
