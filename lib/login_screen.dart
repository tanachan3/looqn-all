...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> signInWithGoogle(BuildContext context) async {
    ...
    try {
      ...
      final loginId = result.googleAccountId?.isNotEmpty == true
          ? result.googleAccountId!
          : user.uid;
      final userDocId = await _upsertUserDoc(
        type: 'google',
        loginId: loginId,
        displayName: '名無し',
      );
      final isPenalty = await PenaltyService.isPenaltyUser(userDocId);
      if (isPenalty) {
        if (!mounted) {
      ...
      await saveCurrentUserDocId(userDocId);
      await _saveFCMToken(userDocId);
  ...
  Future<void> signInWithApple(BuildContext context) async {
    ...
    try {
      ...
      final loginId = appleCredential.userIdentifier?.isNotEmpty == true
  ...
  Future<String> _upsertUserDoc({
    required String type,
    required String loginId,
    required String displayName,
  }) async {
    final docId = buildUserDocId(type: type, loginId: loginId);
    final userDoc =
